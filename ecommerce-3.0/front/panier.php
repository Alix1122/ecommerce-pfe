    ...
    <div class="container py-2">
        ...
        if (isset($_POST['valider'])) {
            ...
            foreach ($produits as $produit) {
                ...
                // Récupérer la quantité (soit nombre soit tableau)
                if (is_array($panier[$idProduit])) {
                    $qty = $panier[$idProduit]['quantite'];
                } else {
        ...
        if (isset($_GET['success'])) {
            ...
            <div class="alert alert-success" role="alert">
                Votre commande avec le montant <strong>(<?php echo $_GET['total'] ?? 0 ?>)</strong> <i
                    class="fa fa-solid fa-dollar"></i> est bien ajoutée.
            </div>
            <hr>
            <?php
        }
                ...
                if (empty($panier)) {
                    ...
                    <table class="table">
                        ...
                        foreach ($produits as $produit) {
                            ...
                            if (is_array($panier[$idProduit])) {
                                $quantite = $panier[$idProduit]['quantite'];
                                $taille = $panier[$idProduit]['taille'] ?? '-';
                            } else {
                                $quantite = $panier[$idProduit];
                                $taille = '-';
                            }
                            $totalProduit = calculerRemise($produit['prix'], $produit['discount']) * $quantite;
                            $total += $totalProduit;
                            ?>
                            <tr>
                                ...
                                <td><?php echo $panier[$idProduit]['taille'] ?? '-' ?></td>
                                <td><strike><?php echo $produit['prix'] ?> <i class="fa fa-solid fa-dollar"></i></strike></td>
                                <td> - <?= $produit['discount'] ?> %</td>
                                <td><?php echo calculerRemise($produit['prix'], $produit['discount']) ?> <i
                                        class="fa fa-solid fa-dollar"></i></td>
